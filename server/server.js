            ...
            localCart.forEach((localItem) => {
                ...
                if (existingDBItem) {
                    localItem.options.forEach((localOption) => {
                        const existingOption = existingDBItem.options.find((option) => option.key === localOption.key);
                        if (existingOption) {
                            if (type === 'update') {
...
app.post('/api/removeCartOption', async (req, res) => {
    ...
    try {
        ...
        user.cart = user.cart
            .map((item) => {
                const updatedOptions = item.options.filter((option) => option.key !== optionKey);
...
app.post('/api/join', async (req, res) => {
    try {
        ...
        const { joinType, userId, password, userName, phone, phone1, phone2, phone3, email, birth, birthYear, birthMonth, birthDay, recommand } = userForm;
