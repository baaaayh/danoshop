    ...
    return (
        <div className={styles['login']}>
            ...
            <div className={`${styles['login__row']}`}>
                ...
                {location.state.prevPage === '/order/cart' || location.state.prevPage === '/order/order' ? (
                    <div className="mt-30">
                        <Link to={'/order/order'} className="btn btn-square btn-square--white">
                            <span className="btn-square__text">비회원 구매</span>
                        </Link>
                    </div>
                ) : null}
            </div>
